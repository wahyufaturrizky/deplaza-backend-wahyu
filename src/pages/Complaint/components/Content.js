...
const DataTable = (props) => {
  ...
  const getReason = () => {
    setLoading(true);
    axiosConfig.get("complaint/reason?limit=1000").then((json) => {
      setReason(json.data.data);
      setLoading(false);
    });
  };
  const fil = reason.filter((data) => data.id === 6);
  ...
  const complaintDetail = async (idData) => {
    await axiosConfig.get(`${URL_DETAIL}/${idData}`).then((res) => {
      setDetail(res.data.data);
    });
    ...
    <div className="content-wrapper">
                      ...
                      {loading === true ? (
                        ...
                        productsData.map((product, i) => (
                          <tr>
                            ...
                            <td>{product.order.seller.fullname}</td>
                            <td>{product.order.customer.fullname}</td>
                            <td>{product.order_id}</td>
                            <td>
                              {reason
                                .filter((data) => data.id === product.reason_id)
                                .map((get) => get.name)}
                            </td>
                            ...
                            <td>{product.qty}</td>
                            <td>{product.order.delivery.receiver_address}</td>
                            <div
                              style={{
                                flexDirection: "row",
                                display: "flex",
                            ...
                            >
                              <button
                                ...
                                onClick={() => complaintDetail(product.id)}
                              >
                                Lihat
                              </button>
                              <button
      ...
      <div className="modal fade" id="modal-detail">
        <div className="modal-dialog modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h4 className="modal-title">Detail Complain</h4>
              <button
                type="button"
                className="close"
                data-dismiss="modal"
            ...
            <div className="modal-body">
              <div className="card-body">
                <div className="form-group">
                  <label htmlFor="exampleInputEmail1">Rincian Alasan</label>
