...
const DataTable = (props) => {
  ...
  const handleAddCategory = () => {
    ...
    axiosConfig
      ...
      .catch((error) =>
        toastr.error("Username / Email / No hp Telah digunakan")
      );
  };
  ...
  const changeData = () => {
    ...
    axiosConfig.post(`${URL_DETAIL}/${id}`, formData).then((res) => {
      getProduct();
  ...
  const handlePageChange = (page, e) => {
    ...
    if (search) {
      axiosConfig
        .get(`${URL_SEARCH}${debouncedSearch}&offset=${offset}&role=0`)
        .then((res) => {
      ...
      axiosConfig
        .get(`${URL_STRING}?role=0&limit=10&offset=${offset}`)
        .then((json) => {
